...
describe('User api tests', () => {
  ...
  test('all users are returned', async () => {
    ...
    logger.debug('test all returned, get resp:', response.body)
    logger.debug('test all returned, saved:', users)
  ...
  test('creation fails statuscode and message if username in use', async () => {
    ...
    const result = await api
      ...
      .expect(400)
      .expect('Content-Type', /application\/json/)
    //logger.debug(result.statusCode, result.body.error)
  ...
  test('creation fails with username length errors', async () => {
    ...
    const newUser = { username: 'ro'.repeat(USERNAME_MIN),
